        ...
        if (result?.response) {
            ...
            if (Array.isArray(resp?.output) && resp.output.length > 0) {
                ...
                if (Array.isArray(out0?.content) && out0.content.length > 0) {
                    const part = out0.content[0]?.text ?? out0.content.map((c) => c?.text ?? '').join('');
 ...
 */
function parseAiResponse(rawResponse, expectedLength) {
    try {
        ...
        const keywords = parsedJson.keywords.map((k) => String(k).trim());
        return keywords;
    }
    catch (error) {
    ...
    while (true) {
        ...
        try {
            ...
            if (!keywords) {
                ...
                const updatePromises = productsToProcess.map((product, index) => prisma.winningProduct.update({
                    where: { id: product.id },
