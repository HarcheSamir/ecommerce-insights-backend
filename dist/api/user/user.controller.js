...
exports.updateUserProfile = exports.getUserNotifications = exports.updatePassword = exports.getUserProfile = void 0;
...
const getUserProfile = async (req, res) => {
    try {
        ...
        const paidTransaction = await index_1.prisma.transaction.count({
            where: {
                userId: userId,
                status: 'succeeded',
            },
        ...
        const userProfile = await index_1.prisma.user.findUnique({
            where: { id: userId },
            select: {
                id: true,
                email: true,
        ...
        const totalSearchCount = await index_1.prisma.searchHistory.count({
            where: { userId: userId },
        });
        ...
        if (userProfile.visitedProfiles.length === 0) {
            const randomCreators = await index_1.prisma.contentCreator.findMany({
                take: 5,
                orderBy: {
